// Adapted from https://github.com/wimdeblauwe/vite-plugin-spring-boot
...
interface SpringBootOptions {
  fullCopyFilePaths?: {
     ...
     * copied, a full page refresh is done. By default, they are equal to `['**\/*.html', '**\/*.png', '**\/*.svg']`
     */
...
export default (options: SpringBootOptions = {}) => {
  const targetDir: string = path.resolve(process.cwd(), 'build')

  const outputDir: string = path.join(targetDir, 'resources/main')

  const devServerConfigOutputDir: string = path.join(targetDir, 'vite-spring-boot-plugin')
  const devServerConfigOutputFile: string = path.join(
    devServerConfigOutputDir,
  ...
  const initializeFilter = (rootDir: string) => {
    if (!filter) {
      filter = createFilter(
        options.fullCopyFilePaths?.include || ['**/*.html', '**/*.png', '**/*.svg'],
 ...
 */
const writeDevServerConfigFile = (config: ResolvedConfig, devServerConfigOutputFile: string) => {
  ...
  fs.writeFileSync(path.resolve(process.cwd(), devServerConfigOutputFile), configData)
}
 ...
 */
const updateDevServerConfigFile = (server: ViteDevServer, configPath: string) => {
...
const copyFile = (src: string, dest: string, logCopy = false) => {
